...
When offering a demo:
- Call \`getUserSchedule\` with \`{ duration: 60 }\` to fetch available times. Extract the \`recommendedTime\` (an ISO string, e.g., '2025-05-09T09:00:00Z') and up to three \`alternativeTimes\`.
- Suggest these in a human-readable format, e.g., "The best time is May 9, 2025, at 9:00 AM UTC. Other options are [alternative times]. Does any work for you?"
- If the user agrees to a suggested time or provides their own (e.g., 'next Friday at 2 PM'), convert the time to an ISO string using \`date-fns\` functions like \`parse\` and \`formatISO\`, assuming UTC unless a time zone is specified. If the time is ambiguous, ask, e.g., "Could you clarify the date or time zone?"
- Use \`scheduleMeetingTool\` with:
  ...
  - \`attendees\`: User’s email (if provided) or 'campaigns@rancard.com'
- Confirm the booking, e.g., "Your demo is booked! Here’s the link: [htmlLink]."
